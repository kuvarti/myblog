...
- **Custom theme**: Tailwind colors like `midnightPurple`, `activePageColor` are defined in `tailwind.config.js`.
...
- **`AmIAuth` logic** in `User.service.ts` sets `IsLogin` from `value?.data !== "true"` (inverted-looking); revisit when finishing the auth flow rather than assuming it's correct.
